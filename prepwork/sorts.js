...
const mergeSort = function(arr) {
  ...
  //spilt values to singular values
  //rebuild and merge back with merge helper function
}
//merge
const merge = function(left, right) {
  ...
  //compare values and add in order to results

}
//bubble sort
const bubbleSort = function(arr) {
  //track whether swap occurs
  let isSwapped = false;
  while (!isSwapped) {
  ...
  return arr;
  //contninue to iterate through arr while a swap occurs
...
const insertionSort = function(arr) {
  ...
  //compares next value to previous value
    //iterate from previous values index backwards
      //check agian and swap
  //return arr
  return arr;
}
//quick sort
const quickSort = function(arr) {
  ...
  //creates a lesser and greater arrays
  //iterates through arr and comapres pivot
    //if less than pivot
      //add to less
    //else
      //add to greater
  //concat values backtogether
}
//counting sort
const countingSort = function(arr) {
  //utilize arr values as indexes
  //values at those indexes represent repeats
  let counts = [];
  for (var i = 0; i < arr.length; i++) {
    if (counts[arr[i]]) {
      counts[arr[i]]++;
  ...
  //create results
  let results = [];
  //iterate through counts arr
  counts.forEach(function(count, num) {
    for (var j = 0; j < count; j++) {
      results.push(num);
    }
  })
  return results;
  //add back to results arr
}
