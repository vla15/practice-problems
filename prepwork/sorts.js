...
//merge sort breaks it down to a singular value
//then rebuilds the array back in order from smeallest to largest
...
const quickSort = function (array) {
  //takes a pivot point
  //compares to create two arrays, one smaller and greater than pivot point
  if (array.length < 2) {
    return array;
  }
  let pivot = array[0];
...
const countingSort = function (array) {
  //use array values as index
  //as values at those index will represent the count;
  let counts = [];
  ...
  let results = [];
  counts.forEach(function(number, count) {
